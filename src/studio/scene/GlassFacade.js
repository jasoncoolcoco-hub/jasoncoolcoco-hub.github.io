...
export function createGlassFacade(materials) {
  ...
  group.userData.structureName = 'Face 1 / Glass Facade on edge A–B'
  ...
  bayStops.forEach((stop, index) => {
    ...
    mullion.userData.structureName = `Wall 1 mullion ${index + 1}`
    group.add(mullion)
  ...
  const bottomRail = box(0.3, 0.18, endZ - startZ, materials.glassFrame)
  bottomRail.position.set(spec.position[0] - 0.04, spec.elevation, (startZ + endZ) / 2)
  bottomRail.name = 'Wall1BottomRailAB'
